...
type
  ...
  private
    ...
    var iCount44, iCount55, iCount66, iCount77 : Integer;
...
begin
  ...
  ledWins.Clear;
  ledWinsP.Clear;
  ledAantaVerkoop.Clear;

  if (rbLays.Checked) or (rbDoritos.Checked) or (rbBigCornBites.Checked) or (rbFritos.Checked) then
...
//------------------------------------------------------------------------------
    ...
    if (rbCoke.Checked) or (rbFanta.Checked) or (rbSprite.Checked) or (rbCreamSoda.Checked) then
      begin
...
//------------------------------------------------------------------------------
      ...
      if (rbMuffin.Checked) or (rbToastie.Checked) or (rbQuiche.Checked) or (rbCookie.Checked) then
        begin
...
//------------------------------------------------------------------------------
        ...
        if (rbMVanilla.Checked) or (rbMStrawberry.Checked) or (rbWMango.Checked) or (rbWNaartjie.Checked) then
...
//------------------------------------------------------------------------------
          ...
          if (iCount44 <> 1 or 2 or 3 or 4) and (iCount55 <> 1 or 2 or 3 or 4) and (iCount66 <> 1 or 2 or 3 or 4) and (iCount44 <> 1 or 2 or 3 or 4) then

            ShowMessage('Kies ŉ produk');

//==============================================================================

  if iCount44 > 0 then
    begin  //Vertoon die statistieke van verskillende produkte in slegs leesbare labled edits
      ledKP.Text := FloatToStrF(rChipsKP,ffCurrency,10,2);
      ledVP.Text := FloatToStrF(rChipsVP,ffCurrency,10,2);
      ledWins.Text := FloatToStrF((rChipsVP - rChipsKP),ffCurrency,10,2);
      ledWinsP.Text := FloatToStrF((((rChipsVP - rChipsKP) / rChipsKP) * 100), ffFixed, 10, 2) + '%';
